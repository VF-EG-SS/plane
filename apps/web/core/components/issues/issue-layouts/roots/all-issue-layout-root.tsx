...
import { WorkspaceIssuesCsvExport } from "@/components/issues/issue-layouts/workspace-csv-export";
...
export const AllIssueLayoutRoot = observer(function AllIssueLayoutRoot(props: Props) {
  ...
  const analyticsFilters = useMemo<TWorkItemFilterExpression | undefined>(() => {
    const serializedFilters = searchParams.get("analytics_filters");
    if (!serializedFilters) return undefined;

    try {
      const parsedFilters: unknown = JSON.parse(serializedFilters);
      return parsedFilters && typeof parsedFilters === "object" && !Array.isArray(parsedFilters)
        ? (parsedFilters as TWorkItemFilterExpression)
        : undefined;
    } catch {
      return undefined;
    }
  ...
  const { isLoading: issuesLoading } = useSWR(
    ...
    async () => {
      if (workspaceSlug && globalViewId) {
        ...
        if (analyticsFilters) await updateFilterExpression(workspaceSlug, globalViewId, analyticsFilters);
            ...
            <div className="flex h-full w-full flex-col border-b border-strong">
              {globalWorkItemsFilter && (
                <WorkItemFiltersRow
                  filter={globalWorkItemsFilter}
                  actions={
                    <WorkspaceIssuesCsvExport
                      issueIds={groupedIssueIds?.[ALL_ISSUES] ?? []}
                      issueMap={issueMap}
                      workspaceSlug={workspaceSlug}
                    />
                  }
