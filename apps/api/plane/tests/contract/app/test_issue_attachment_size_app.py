...
class TestSessionIssueAttachmentSize:
    ...
    def test_exact_limit_is_presigned_without_clamping(self, session_client, workspace, project, issue):
    ...
    @pytest.mark.django_db
    def test_above_limit_returns_413_without_creating_asset(self, session_client, workspace, project, issue):
        with mock.patch("plane.app.views.issue.attachment.S3Storage") as storage:
            response = session_client.post(
                self.url(workspace, project, issue),
    ...
    def test_invalid_size_returns_400_without_creating_asset(self, session_client, workspace, project, issue, size):
        response = session_client.post(
            self.url(workspace, project, issue),
...
def upload_initiator(db):
    return User.objects.create(
        email="session-upload-initiator@example.com",
        username="session-upload-initiator",
    )
...
@pytest.mark.contract
class TestSessionIssueAttachmentConfirmation:
    ...
    def test_matching_upload_is_confirmed_once_and_preserves_creator(
        self, session_client, workspace, project, issue, pending_attachment, upload_initiator
    ):
        metadata = {
            "ContentLength": 1024,
        ...
        with (
            ...
            mock.patch("plane.app.views.issue.attachment.issue_activity") as activity,
        ):
            storage.return_value.get_object_metadata.return_value = metadata
            first_response = session_client.patch(url, {"is_uploaded": True}, format="json")
            second_response = session_client.patch(url, {"is_uploaded": True}, format="json")
    ...
    def test_invalid_storage_metadata_leaves_upload_unconfirmed(
        self, session_client, workspace, project, issue, pending_attachment, metadata, error
    ):
        with (
            mock.patch("plane.utils.issue_attachment_upload.S3Storage") as storage,
            mock.patch("plane.app.views.issue.attachment.issue_activity") as activity,
    ...
    def test_already_uploaded_attachment_does_not_access_storage_or_emit_activity(
        self, session_client, workspace, project, issue, pending_attachment
    ):
